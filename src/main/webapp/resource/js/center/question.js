...
{  
	...
	{   
		total_page=0;
		$("#table_id").empty();
		$("#table_id").html("<thead><th>ID</th><th>题目</th><th>浏览次数</th><th>发布者</th><th>评论数</th><th>操作</th></thead>");
		$.each(data, function(){
			 $("#table_id").append("<tr><td>"+this.id+"</td><td><a href='/question_square/questions/"+this.id+"'>"+this.title+"</a></td>" +
			 		"<td>"+this.clickTimes+"</td><td>"+this.user+"</td><td>"+this.countComment+"</td><td><button class='btn btn-success'>屏蔽</button></td></tr>");
